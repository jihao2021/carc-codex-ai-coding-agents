...
# catches something real, it does not belong here — put it in AGENTS.md
# instead, where it's guidance rather than enforcement.
#
# Codex hook contract (abridged):
...
#     user prompt even when permission mode would auto-allow.

set -u

# ---------- Constants ----------

readonly LOG_DIR_PRIMARY="/var/log/codex-carc"
readonly LOG_DIR_FALLBACK="$HOME/.codex/audit"
...
print("\x1f".join([
    s(d.get("tool_name") or d.get("tool") or d.get("name")),
    s(ti.get("command") or ti.get("cmd") or ti.get("shell_command")),
    s(ti.get("file_path") or ti.get("path") or ti.get("filename")),
    s(d.get("session_id")),
    s(d.get("cwd")),
]))
' 2>/dev/null)
...
case "$TOOL" in
    Bash|exec_command|shell|unified_exec|functions.exec_command)
        TOOL="Bash"
        ;;
esac
...
log_event() {
    ...
    local ts; ts=$(date -u +"%Y-%m-%dT%H:%M:%SZ" 2>/dev/null || date 2>/dev/null || echo unknown-time)
    mkdir -p "$(dirname "$LOG_FILE")" 2>/dev/null || true
    # Single line per event — jq-friendly if anyone ever wants to parse it.
    printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
        "$ts" "$host" "$USER" "$decision" "$TOOL" "$reason" "$CMD" \
...
credential_shape_reason() {
    ...
    # macOS still ships Bash 3.2, which lacks ${var,,}; keep the hook
    # portable so admins can run the test harness from laptops too.
    local lower
...
emit_ask() {
    ...
    printf '[codex-precheck] asking for confirmation: %s\n' "$reason" >&2
    python3 -c '
import json, sys
print(json.dumps({
    "hookSpecificOutput": {
...
block() {
    local reason="$1"
    printf '[codex-precheck] BLOCKED: %s\n' "$reason" >&2
    printf 'Command: %s\n' "$CMD" >&2
...
if [ "$TOOL" != "Bash" ]; then
    # Write / Edit / MultiEdit to sensitive paths. The sandbox may catch some
    # cases too; this hook gives clearer CARC-specific messages.
    case "$TOOL" in
        Write|Edit|MultiEdit|NotebookEdit)
            # Adding/altering an SSH login key grants access to the account —
            # never something Codex should do unattended.
            case "$(abspath_of "$FILE_PATH")" in
                "$HOME"/.ssh/authorized_keys|"$HOME"/.ssh/authorized_keys2)
                    block "writing to ~/.ssh/authorized_keys would grant SSH login to your account. Codex must not add login keys — if you intend to, do it yourself." ;;
            esac
            ...
            ;;
        Read)
            # These branches catch the long tail:
...
# 3. /project2 group-membership check.
#    For any destructive op that names a path under /project2/<subdir>/, the
#    <subdir> must be one of the user's groups (id -Gn). 777 leftovers from
#    other labs do not grant permission by policy.
check_project2_paths() {
    local verb_re='(rm|chmod|chown|mv|cp)[[:space:]]+(-[^ ]+[[:space:]]+)*'
    # Extract every /project2/<subdir> token from $EXPANDED.
    # python scan is cleaner than layered grep.
    local subdirs
    subdirs=$(printf '%s' "$EXPANDED" | python3 -c '
import re, sys
s = sys.stdin.read()
# Match destructive verbs followed (after flags) by a /project2/<sub>/... token.
# We are intentionally permissive about flag parsing: we just scan every
# /project2/<sub> that appears anywhere in a line containing a destructive verb.
dangerous = re.compile(r"\b(rm|chmod|chown|mv)\b")
for line in s.splitlines():
    if not dangerous.search(line):
        continue
    for m in re.finditer(r"/project2/([A-Za-z0-9._-]+)", line):
        print(m.group(1))
' | sort -u)
...
# 4. /scratch1 user-ownership check — same spirit as project2, keyed on user.
check_scratch1_paths() {
    local users
    users=$(printf '%s' "$EXPANDED" | python3 -c '
import re, sys
s = sys.stdin.read()
dangerous = re.compile(r"\b(rm|chmod|chown|mv)\b")
for line in s.splitlines():
    if not dangerous.search(line):
        continue
    for m in re.finditer(r"/scratch1/([A-Za-z0-9._-]+)", line):
        print(m.group(1))
' | sort -u)
    for u in $users; do
        if [ "$u" != "$USER" ]; then
...
if is_login_node; then
    ...
    #     framing, so Codex doesn't pre-empt the prompt with a salloc recipe.
...
#     Write/Edit tool path may also be sandboxed; this is the shell-redirect
#     path. Read-only commands (cat/grep/diff with no write
...
if has_pattern '(~|\$HOME|\$\{HOME\}|/home1?/[A-Za-z0-9._-]+|/root)/\.ssh/authorized_keys2?([^A-Za-z0-9_]|$)' \
   && has_pattern '(>>?|(^|[[:space:];&|(])(tee|cp|install|rsync|dd|sed)([[:space:]]|$))'; then
    block "this writes to ~/.ssh/authorized_keys — that grants SSH login to the account. Codex must not add login keys; do it yourself if intended."
fi
...
#     cat/head/tail/etc. The Read-tool branch above gates file-read tool calls;
#     this catches the same shapes when Codex shells out. Best-effort: after
